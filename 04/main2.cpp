...
int main() {
  ...
  // Now we need to calculate how many boards have won already
  int boards_to_win = num_of_lines / 5;
  ...
  for (int inst : instructions) {
	...
	// Mark number
	// -2 indicates an already won board
	if(int_boards[i] == -2) continue;
	int_boards[i] = -1;
	...
	for (int j = 0; j < 25; j++) {
	  ...
	  int_boards[start_board_index + j] = -2;
	}
      }
    }
