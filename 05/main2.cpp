...
int main() {
  ...
  for (instruction inst : instructions) {
	...
	y_incrementer = 1;
      ...
      // Start From (X1, Y1) and increment by -1 or 1 depening on direction
      // until (X2, Y2) is hit
      int y1 = inst.y1;
      for (int x1 = inst.x1; x1 != inst.x2; x1 += x_incrementer) {
	increment_value_in_2D_array(coords_mem, x1, biggest_x, y1);
	y1 += y_incrementer;
      }
      // One last time for (X2, Y2)
      increment_value_in_2D_array(coords_mem, inst.x2, biggest_x, inst.y2);
    }
  }
