...
// for 40 steps way bigger numbers are needed
typedef unsigned long long u64;

struct entry {
  bool initialized;
  u64 char_count[NUM_OF_LETTERS];
};
...
void
merge_char_counts(u64* merge_to, u64* merge_from)
{
			    ...
			    int depth, entry* dp_table)
{
  ...
  u64* char_count1 = recursive_polymer_insertion(a, to_insert, lookup_table, depth + 1, dp_table);
  u64* char_count2 = recursive_polymer_insertion(to_insert, b, lookup_table, depth + 1, dp_table);
...
int main(int argc, char** argv) {
  ...
  u64 char_counts[NUM_OF_LETTERS] = {0};
  u64* char_counts_ptr = char_counts;
  ...
  for (size_t i = 0; i < current_polymer.length() - 1; i++) {
    ...
    u64* result_ptr = recursive_polymer_insertion((int)current_polymer[i] - 65,
